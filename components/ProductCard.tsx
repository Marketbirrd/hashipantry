      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          {product.dietTags.slice(0, 3).map((tag) => (
            <span key={tag} className="text-[10px] bg-sage-pale text-forest/70 px-1.5 py-0.5 rounded-full">
              {tag}
            </span>
          ))}
        </div>
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-1.5 text-xs font-semibold py-2.5 rounded-xl transition-colors ${
            inCart || added ? "bg-sage text-white" : "bg-forest text-white hover:bg-forest-light"
          }`}
        >
